...
#include "aclrtlaunch_apply_token_bitmask_legacy_fp32.h"
#include "aclrtlaunch_apply_token_bitmask_legacy_fp16.h"
#include "aclrtlaunch_apply_token_bitmask_legacy_bf16.h"
#include "tiling/platform/platform_ascendc.h"

namespace sglang {
namespace npu_kernel {

at::Tensor apply_token_bitmask_impl(at::Tensor logits, at::Tensor bitmask, c10::optional<at::Tensor> indices,
                                    bool useLegacyPartition)
{
    // Input validation
    ...
    if (!useLegacyPartition) {
        ...
        tileLength = static_cast<uint32_t>(std::min<int64_t>(maxTileLength, targetTileLength));
    }

    int64_t numTiles = (paddedVocabSize + tileLength - 1) / tileLength;
    int64_t totalTiles = numIndices * numTiles;
    int64_t blocks =
        useLegacyPartition ? std::min<int64_t>(numIndices, coreNum) : std::min<int64_t>(totalTiles, coreNum);
    uint32_t blockDim = static_cast<uint32_t>(std::max<int64_t>(blocks, 1));

    // The optimized kernel ignores row partitioning, but keeping one launch
    // signature makes the A/B comparison differ only in scheduling.
    ...
    if (dtype == at::kFloat) {
        if (useLegacyPartition) {
            EXEC_KERNEL_CMD(apply_token_bitmask_legacy_fp32, blockDim, workingLogits, workingBitmask, numRowsU32,
                            vocabSizeU32, logitsStrideU32, bitmaskStrideU32, baseRows, extraCores, tileLength, blockDim,
                            dtypeSizeU32);
        } else {
            EXEC_KERNEL_CMD(apply_token_bitmask_fp32, blockDim, workingLogits, workingBitmask, numRowsU32, vocabSizeU32,
                            logitsStrideU32, bitmaskStrideU32, baseRows, extraCores, tileLength, blockDim,
                            dtypeSizeU32);
        }
    } else if (dtype == at::kHalf) {
        if (useLegacyPartition) {
            EXEC_KERNEL_CMD(apply_token_bitmask_legacy_fp16, blockDim, workingLogits, workingBitmask, numRowsU32,
                            vocabSizeU32, logitsStrideU32, bitmaskStrideU32, baseRows, extraCores, tileLength, blockDim,
                            dtypeSizeU32);
        } else {
        ...
        if (useLegacyPartition) {
            EXEC_KERNEL_CMD(apply_token_bitmask_legacy_bf16, blockDim, workingLogits, workingBitmask, numRowsU32,
                            vocabSizeU32, logitsStrideU32, bitmaskStrideU32, baseRows, extraCores, tileLength, blockDim,
                            dtypeSizeU32);
        } else {
...
{
    return apply_token_bitmask_impl(logits, bitmask, indices, false);
}

HOST_API at::Tensor apply_token_bitmask_legacy(at::Tensor logits, at::Tensor bitmask, c10::optional<at::Tensor> indices)
{
    return apply_token_bitmask_impl(logits, bitmask, indices, true);
}
